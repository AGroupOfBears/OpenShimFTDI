...
public class ISORead : ISOMain
{
	...
	public static void walbroSetTPS(int offset, int value)
	{
		Walbro_Set_Value[offset + 12] = (byte)(value >> 8);
		Walbro_Set_Value[offset + 13] = (byte)(value & 0xFF);
	}
	...
	public static bool readSerial(byte[] data, int start, int length)
	{
		...
		case eMode.MODE_NULL:
			...
			else if (text.StartsWith(":OK!"))
			{
				...
				{
					int num3 = (mLoad ? readLen : IMap.flashSize);
					ISOMain.enableMenuItems(mMode, 0);
					ISOMain.DisplayMsg(ISOMain.LangUI[ISOMain.mLang, mLoad ? 217 : 213], 32);
					ISOMain.ProgressBarInit(num3 / ISOMain.sBloc);
		...
		case eMode.MODE_WALBRO_VERSION:
			if (text.StartsWith(":OK!"))
			{
				...
				int num = IMap.CheckMapID(ECUTyp, 1) >> 16;
				if (num > 0)
				{
					rIdle = IMap.getAddr(num, 5) | 0x10000;
					rMS = IMap.getAddr(num, 36) | 0xFF0000;
					num = (((ECUTyp & 0x4100) == 16640) ? 10 : 8);
					Walbro_TPS[9] = (byte)byte2String((byte)num);
					addrRead = 2;
					readRow = 0;
					ISOMain.RevCount = 2;
		...
		case eMode.MODE_WALBRO_ERASING:
			...
			else if ((length == 2) & (data[start] == 4))
			{
				uBuffer[0] = 8;
				int num4 = data[1];
				ISOMain.DisplayMsg(ISOMain.LangUI[ISOMain.mLang, 347] + num4 + ISOMain.LangUI[ISOMain.mLang, 348], 32);
				if (num4 == 15)
				{
