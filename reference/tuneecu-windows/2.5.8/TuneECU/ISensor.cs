...
public class ISensor : ISORead
{
	...
	public static void DataSensorUpdate(byte[] msg, int start, int length)
	{
		...
		else if (length == 9)
		{
			...
			if (ISOMain.setTPS)
			{
				if (ISOMain.tpsMin > num2)
				{
					ISOMain.tpsMin = num2;
				}
				if (ISOMain.tpsMax < num2)
				{
					ISOMain.tpsMax = num2;
				}
			}
		...
		if ((ISOMain.swMode == 2) & (ISORead.mMode == eMode.MODE_WALBRO_SENSORS) & (length < 0))
		{
			...
			ISORead.dataSensor[2] = (num / 10.0).ToString("##0.0");
			...
			ISORead.dataNum[2] = num3;
			ISORead.dataSensor[54] = ((double)ISORead.dataNum[(num3 == 0) ? 19 : 21] / 128.0).ToString("0.000");
		}
	}
