...
.PHONY: all clean test test-loopback test-ipc test-helper test-break-decode test-packetizer

all: openshim-helper FTD2XX.dll test_ipc_framing test_helper_live test_shim.exe test/test_shim_ipc.exe test/test_shim_break_decode.exe test/test_shim_packetizer.exe
...
test/test_shim_packetizer.exe: test/test_shim_packetizer.c FTD2XX.dll
	$(CLANG_WIN) $(WIN_CFLAGS) -c test/test_shim_packetizer.c -o test/test_shim_packetizer.o
	$(CLANG_WIN) $(WIN_LDFLAGS) -o $@ test/test_shim_packetizer.o -Wl,-e,_main -Wl,--subsystem,console
...
test-packetizer: FTD2XX.dll test/test_shim_packetizer.exe
	@echo "=== Running stream packetizer & local echo tests ==="
	WINEDLLOVERRIDES="ftd2xx=n" OPENSHIM_BACKEND=loopback $(WINE) ./test/test_shim_packetizer.exe
...
test: test_ipc_framing test-loopback test-break-decode test-packetizer test-helper test-ipc
	@echo "=== Running IPC framing unit test ==="
	./test_ipc_framing
	@echo "=== ALL TEST SUITES PASSED ==="

clean:
	rm -f openshim-helper FTD2XX.dll test_ipc_framing test_helper_live test_shim.exe test/test_shim_ipc.exe test/test_shim_break_decode.exe test/test_shim_packetizer.exe test_five_baud_live
