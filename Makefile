...
.PHONY: all clean test test-loopback test-ipc test-helper

all: openshim-helper FTD2XX.dll test_ipc_framing test_helper_live test_shim.exe test/test_shim_ipc.exe
...
test: test_ipc_framing test-loopback test-helper test-ipc
	@echo "=== Running IPC framing unit test ==="
	./test_ipc_framing
	@echo "=== ALL TEST SUITES PASSED ==="

clean:
	rm -f openshim-helper FTD2XX.dll test_ipc_framing test_helper_live test_shim.exe test/test_shim_ipc.exe
